<link rel="stylesheet" href="../css/nav_bar.css">

<nav class="navbar navbar-expand-lg ">
  <div class="container-fluid">
    <a class="navbar-brand" href="#">Zietnam Club ADMIN</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link" aria-current="page" href="../pages/index.php">H O M E</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="../pages/study_page.php">U S E R _ M A N A G E</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="../pages/settings_page.php">C L A S S</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="../pages/settings_page.php">S E T T I N G S</a>
        </li>
        
      </ul>
        <div class="header-main-sm">
            <a href="https://www.facebook.com/saigontranslatorteambyjulie"><i class="fa-brands fa-facebook"></i></a>
            <a href="https://www.xiaohongshu.com/user/profile/627362150000000021020c68"><div class="header-main-sm-in"></div></a>
            <i id="change-mode" class="fas fa-moon"></i>

        </div>
    </div>
  </div>
</nav>
<div class="content">
  </div>
<script src="../js/header.js"></script>
<script src="../js/dark-mode.js"></script>